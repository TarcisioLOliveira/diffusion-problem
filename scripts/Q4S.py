...
def init_B():
    ...
    B = sympy.Matrix([NN.diff(xi), NN.diff(eta)])
...
def make_diffusion_2D():
    """
        Creates the elemental diffusion matrix with constant A.
    """
    init_B()

    A = sympy.symbols("A:4")
    AA = sympy.Matrix([[A[0], A[1]],
                       [A[2], A[3]]])
    ...
    for i in range(len(k)):
        ...
        formatted = re.sub(r"([A]\d)\*\*2", r"\1*\1", formatted)
        formatted = re.sub(r"([A])(\d)", r"\1[\2]", formatted)
