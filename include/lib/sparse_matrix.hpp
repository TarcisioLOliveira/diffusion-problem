...
#ifndef DPLIB_SPARSE_MATRIX_HPP
#define DPLIB_SPARSE_MATRIX_HPP
...
class SparseMatrix{
    ...
    inline void get_band_dimensions(size_t& L, size_t& W) const{
        L = this->L;
        if(this->ku == 0){
            W = 2*this->kl + 1;
        } else if(this->kl == 0){
    ...
    inline void get_band_dimensions_symmetric(size_t& L, size_t& W) const{
        L = this->L;
        W = this->ku + this->kl + 1;
    }
    ...
    std::vector<std::ptrdiff_t> eigen_resize_vector();
    // One-indexed
    ...
    // Does not calculate ku/kl either
    ...
    inline void to_eigen_sparse(Eigen::SparseMatrix<A, B, C>& K) const{
